...
/// annotation table. Callers must collapse `None` into the Unknown error
/// shape (500, empty reason) — matching Go's `FromError` for non-status
/// errors — via `StatusError::new(500, "", ..)`.
pub fn error_status(package: &str, reason: &str) -> Option<i32> {
    crate::gen::error_tables::ERROR_STATUS
        .iter()
