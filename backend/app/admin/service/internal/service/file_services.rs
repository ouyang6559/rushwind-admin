        ...
        let row = match &req.selector {
            ...
            Some(gen_rust::proto::storage::service::v1::download_file_request::Selector::StorageObject(obj)) => {
                crate::data::files::Entity::find()
                    .filter(
                        Condition::all()
                            .add(crate::data::files::Column::BucketName.eq(obj.bucket_name.clone().unwrap_or_default()))
                            .add(crate::data::files::Column::LinkUrl.eq(obj.object_name.clone().unwrap_or_default())),
                    )
                    .one(&self.state.db)
                    .await
                    .map_err(db_err)?
