...
fn log_proto(r: crate::data::sys_operation_audit_logs::Model) -> OperationAuditLog {
    OperationAuditLog {
        id: Some(r.id),
...
impl proto::gen::services::OperationAuditLogServiceHandlers for OperationAuditLogService {
    async fn list(
        &self,
    ...
    async fn get(
        ...
        let Some(proto::proto::audit::service::v1::get_operation_audit_log_request::QueryBy::Id(
            id,
        )) = req.query_by
        else {
        ...
        let row = crate::data::sys_operation_audit_logs::Entity::find_by_id(id)
            .one(&self.state.db)
            .await
            .map_err(db_err)?
